  ...
  const LifeGuidanceView = () => {
    ...
    return (
      <div className="px-5 pt-4 pb-20 space-y-4">
        ...
        <div className="flex flex-nowrap gap-1 overflow-x-auto no-scrollbar py-1">
          {['All', 'Marriage', 'Addiction', 'Doubt', 'Family', 'Faith'].map((filter) => (
            <button 
              key={filter}
              onClick={() => setGuidanceFilter(filter)}
              className={`px-2.5 py-1.25 rounded-md text-[7px] font-black uppercase tracking-tighter transition-all border whitespace-nowrap ${
